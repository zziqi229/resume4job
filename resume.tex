...
\newcommand{\resumeItemListStart}{\vspace{-10pt}\begin{itemize}[left=0.2em]}
...
\section{实习经历}
    \begin{itemize}[leftmargin=*,itemsep=-10pt]
        \resumeSubheadingtwo
            ...
            \\[7pt]
            % \resumeItem{开发}
            作为核心成员，负责弹性算力平台的开发和维护。基于云原生技术栈，简化业务场景和资源平台的复杂性，为各类计算负载提供一致的接口，实现低成本、高吞吐量计算
            \resumeItemListStart
                 \resumeItem{}
                {基于 Kubernetes、Virtual Kubelet 实现容器化计算，支持对接多个算力池，提供统一 FaaS 高通量任务计算，接入 $70w$ 核 CPU、$3K$ 张 GPU，并行$20w$任务，多为低成本潮汐资源.}
                \resumeItem{}
                {任务调度容忍容器失效，支持多个容器负载均衡，混合资源提升通量，保证任务 At-Least-Once 和 FIFO 执行.}
                \resumeItem{}
                {容器调度支持动态扩缩容、混合调度和优先级抢占，调度容器到任务最多的机房，容忍潮汐资源的不稳定性}
                \resumeItem{}
                {支持任务配置超时时间和重试次数，任务失败换容器重试，支持批量停止、重启、查询任务.}
            \resumeItemListEnd
    \end{itemize}
    ...
    \begin{itemize}[leftmargin=*,itemsep=5pt]
        \resumeSubheadingtwo
            ...
            \resumeItemListStart
                ...
                \resumeItem{键值数据库}
                    {基于Raft共识构建高可用的分片键值数据库，支持不停机扩缩数据节点、分片动态迁移、过期分片回收、配置热更新，容忍网络丢包、网络分区、节点失效等错误下实现线性一致性.
                        % \begin{itemize}
...
\section{学术经历}
    \begin{itemize}[leftmargin=*,itemsep=-20pt]
        \resumeSubheadingtwo
            ...
            \\[5pt]
            共同一作，首次在多行为推荐中引入多兴趣学习，挖掘用户与物品交互背后的兴趣，提出使用知识图谱提取兴趣的初始表示，结合了动态路由方法进一步探索每个行为背后的兴趣，在多个数据集上达到了最佳的推荐效果.
